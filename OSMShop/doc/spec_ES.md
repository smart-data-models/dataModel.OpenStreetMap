...
<!-- /15-License -->  
<!-- 20-Description -->  
Descripción global: **Una tienda o establecimiento comercial de OpenStreetMap etiquetado con shop=*. Representa establecimientos que venden productos minoristas directamente a los clientes finales**  
version: 0.0.1  
<!-- /20-Description -->  
<!-- 30-PropertiesList -->  
...
- `alternateName[string]`: Un nombre alternativo para este elemento  - `areaServed[string]`: El área geográfica donde se proporciona un servicio o artículo ofrecido  . Modelo: [https://schema.org/Text](https://schema.org/Text)- `brand[string]`: El nombre de la marca si la tienda forma parte de una cadena o franquicia  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:brand](https://wiki.openstreetmap.org/wiki/Key:brand)- `dataProvider[string]`: Una secuencia de caracteres que identifica al proveedor de la entidad de datos armonizada  - `dateCreated[date-time]`: Marca de tiempo de creación de la entidad. Esto generalmente será asignado por la plataforma de almacenamiento  - `dateModified[date-time]`: Marca de tiempo de la última modificación de la entidad. Esto generalmente será asignado por la plataforma de almacenamiento  - `description[string]`: Una descripción de este elemento  - `id[*]`: Identificador único de la entidad  - `location[*]`: Referencia Geojson al elemento. Puede ser Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `name[string]`: El nombre de este elemento  - `openingHours[string]`: Horario de apertura de la tienda en formato OSM  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:opening_hours](https://wiki.openstreetmap.org/wiki/Key:opening_hours)- `operator[string]`: Nombre de la entidad que opera la tienda  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:operator](https://wiki.openstreetmap.org/wiki/Key:operator)- `organic[string]`: Indica si la tienda vende productos orgánicos  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:organic](https://wiki.openstreetmap.org/wiki/Key:organic)- `osmId[number]`: El identificador entero original del elemento OpenStreetMap  - `osmLastModified[date-time]`: Marca de tiempo de la última modificación  - `osmType[string]`: El tipo del elemento OSM original  - `owner[array]`: Una lista que contiene una secuencia codificada en JSON de caracteres que hacen referencia a los IDs únicos del(los) propietario(s)  - `phone[string]`: Número de teléfono de la tienda  - `seeAlso[*]`: Lista de URIs que apuntan a recursos adicionales  - `shopType[string]`: La etiqueta de tienda OSM que clasifica la categoría minorista. Enum: 'supermarket, convenience, bakery, clothes, shoes, hairdresser, car_repair, bicycle, hardware, electronics, florist, books...'  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:shop](https://wiki.openstreetmap.org/wiki/Key:shop)- `source[string]`: Una secuencia de caracteres que proporciona la fuente original de los datos de la entidad como una URL. Se recomienda que sea el nombre de dominio completo del proveedor de la fuente o la URL del objeto fuente  - `type[string]`: Tipo de Entidad NGSI. Debe ser OSMShop  - `website[uri]`: URL del sitio web oficial  - `wheelchair[string]`: Accesibilidad para sillas de ruedas  . Modelo: [https://wiki.openstreetmap.org/wiki/Key:wheelchair](https://wiki.openstreetmap.org/wiki/Key:wheelchair)<!-- /30-PropertiesList -->  
...
#### Ejemplo normalizado NGSI-v2 de OSMShop    
Aquí hay un ejemplo de un OSMShop en formato JSON como normalizado. Esto es compatible con NGSI-v2 cuando no se usan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>mostrar/ocultar ejemplo</strong></summary>    
```json  
...
#### Ejemplo normalizado NGSI-LD de OSMShop    
Aquí hay un ejemplo de un OSMShop en formato JSON-LD como normalizado. Esto es compatible con NGSI-LD cuando no se usan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>mostrar/ocultar ejemplo</strong></summary>    
```json  
