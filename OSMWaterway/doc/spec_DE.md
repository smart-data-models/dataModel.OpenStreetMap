...
<!-- /15-License -->
<!-- 20-Description -->
Globale Beschreibung: **Ein Wasserweg von OpenStreetMap, getaggt mit waterway=*. Beschreibt Flüsse, Bäche, Kanäle, Abflüsse und andere Wasserläufe**
version: 0.0.1
<!-- /20-Description -->
<!-- 30-PropertiesList -->
...
- `address[object]`: Die Postanschrift. Modell: [https://schema.org/address](https://schema.org/address) - `addressCountry[string]`: Das Land. Zum Beispiel Spanien. Modell: [https://schema.org/addressCountry](https://schema.org/addressCountry)
	...
	- `district[string]`: Ein Bezirk ist eine Art Verwaltungsgliederung, die in einigen Ländern von der lokalen Regierung verwaltet wird.
...
- `alternateName[string]`: Ein alternativer Name für diesen Eintrag. - `areaServed[string]`: Das geografische Gebiet, in dem ein Dienst oder ein angebotener Artikel bereitgestellt wird. Modell: [https://schema.org/Text](https://schema.org/Text)- `boat[string]`: Navigationsregeln für Boote auf diesem Wasserweg. Modell: [https://wiki.openstreetmap.org/wiki/Key:boat](https://wiki.openstreetmap.org/wiki/Key:boat)- `dataProvider[string]`: Eine Zeichenfolge, die den Anbieter der harmonisierten Dateneinheit identifiziert. - `dateCreated[date-time]`: Zeitstempel der Erstellung der Entität. Dieser wird normalerweise von der Speicherplattform zugewiesen. - `dateModified[date-time]`: Zeitstempel der letzten Änderung der Entität. Dieser wird normalerweise von der Speicherplattform zugewiesen. - `description[string]`: Eine Beschreibung dieses Eintrags. - `id[*]`: Eindeutiger Identifikator der Entität. - `intermittent[boolean]`: Gibt an, ob der Wasserweg nur zu bestimmten Zeiten fließt (z. B. Regenzeit). Modell: [https://wiki.openstreetmap.org/wiki/Key:intermittent](https://wiki.openstreetmap.org/wiki/Key:intermittent)- `location[*]`: Geojson-Referenz zum Eintrag. Es kann Point, LineString, Polygon, MultiPoint, MultiLineString oder MultiPolygon sein. - `name[string]`: Der Name dieses Eintrags. - `oneway[boolean]`: Gibt an, ob der Schiffsverkehr auf die Richtung des gezeichneten Weges beschränkt ist. Modell: [https://wiki.openstreetmap.org/wiki/Key:oneway](https://wiki.openstreetmap.org/wiki/Key:oneway)- `osmId[number]`: Die ursprüngliche ganzzahlige ID des OpenStreetMap-Elements. - `osmLastModified[date-time]`: Zeitstempel der letzten Änderung. - `osmType[string]`: Der Typ des ursprünglichen OSM-Elements. - `owner[array]`: Eine Liste, die eine JSON-kodierte Zeichenfolge enthält, die auf die eindeutigen IDs des/der Eigentümer(s) verweist. - `salt[boolean]`: Gibt an, ob das Wasser salzhaltig ist. Modell: [https://wiki.openstreetmap.org/wiki/Key:salt](https://wiki.openstreetmap.org/wiki/Key:salt)- `seeAlso[*]`: Liste von URIs, die auf zusätzliche Ressourcen verweisen. - `source[string]`: Eine Zeichenfolge, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Es wird empfohlen, den vollständig qualifizierten Domänennamen des Quellanbieters oder die URL zum Quellobjekt zu verwenden. - `type[string]`: NGSI-Entitätstyp. Es muss OSMWaterway sein. - `waterwayType[string]`: Der OSM-Wasserweg-Tag, der den Wasserlauf klassifiziert. Enum: 'river, stream, canal, drain, ditch, wadi, fairway, dock, dam, weir, waterfall...' Modell: [https://wiki.openstreetmap.org/wiki/Key:waterway](https://wiki.openstreetmap.org/wiki/Key:waterway)- `widthMeters[number]`: Breite des Wasserwegs in Metern. Modell: [https://wiki.openstreetmap.org/wiki/Key:width](https://wiki.openstreetmap.org/wiki/Key:width)<!-- /30-PropertiesList -->
...
Alphabetisch sortiert (zum Details klicken)
<!-- /50-DataModelHeader -->
<!-- 60-ModelYaml -->
...
<!-- 80-Examples -->
## Beispiel-Payloads
#### OSMWaterway NGSI-v2 Schlüssel-Werte-Beispiel
Hier ist ein Beispiel für eine OSMWaterway im JSON-Format als Schlüssel-Werte. Dies ist mit NGSI-v2 kompatibel, wenn `options=keyValues` verwendet wird und gibt die Kontextdaten einer einzelnen Entität zurück.
...
#### OSMWaterway NGSI-v2 normalisiertes Beispiel
Hier ist ein Beispiel für eine OSMWaterway im JSON-Format als normalisiert. Dies ist mit NGSI-v2 kompatibel, wenn keine Optionen verwendet werden und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel anzeigen/ausblenden</strong></summary>
```json  
...
#### OSMWaterway NGSI-LD Schlüssel-Werte-Beispiel
Hier ist ein Beispiel für eine OSMWaterway im JSON-LD-Format als Schlüssel-Werte. Dies ist mit NGSI-LD kompatibel, wenn `options=keyValues` verwendet wird und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel anzeigen/ausblenden</strong></summary>
```json  
...
#### OSMWaterway NGSI-LD normalisiertes Beispiel
Hier ist ein Beispiel für eine OSMWaterway im JSON-LD-Format als normalisiert. Dies ist mit NGSI-LD kompatibel, wenn keine Optionen verwendet werden und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel anzeigen/ausblenden</strong></summary>
```json  
