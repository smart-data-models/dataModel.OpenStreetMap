...
Globale Beschreibung: **Ein spezielles Element der Fahrradinfrastruktur aus OpenStreetMap. Stellt Radwege, gemeinsame Fahrradspuren und Fahrradrouten dar, die mit highway=cycleway oder cycleway=* auf Straßen gekennzeichnet sind.**  
...
- `cyclewayType[string]`: Klassifizierung der Fahrradinfrastruktur. Enum:'lane, track, opposite, opposite_lane, shared_busway, path, separate, shared'. Modell: [https://wiki.openstreetmap.org/wiki/Key:cycleway](https://wiki.openstreetmap.org/wiki/Key:cycleway) 
...
- `lit[boolean]`: Gibt an, ob die Radroute Straßenbeleuchtung hat. Modell: [https://wiki.openstreetmap.org/wiki/Key:lit](https://wiki.openstreetmap.org/wiki/Key:lit) 
...
- `ncnRef[string]`: Referenznummer der nationalen Radroute. Modell: [https://wiki.openstreetmap.org/wiki/Key:ncn_ref](https://wiki.openstreetmap.org/wiki/Key:ncn_ref) 
- `oneway[boolean]`: Gibt an, ob die Radroute für Radfahrer einseitig ist. Modell: [https://wiki.openstreetmap.org/wiki/Key:oneway](https://wiki.openstreetmap.org/wiki/Key:oneway) 
- `operator[string]`: Name der für die Wartung dieser Radroute verantwortlichen Entität. Modell: [https://wiki.openstreetmap.org/wiki/Key:operator](https://wiki.openstreetmap.org/wiki/Key:operator) 
...
- `owner[array]`: Eine Liste, die eine JSON-codierte Folge von Zeichen enthält, die die eindeutigen IDs der Eigentümer referenzieren   
- `rcnRef[string]`: Referenznummer der regionalen Radroute. Modell: [https://wiki.openstreetmap.org/wiki/Key:rcn_ref](https://wiki.openstreetmap.org/wiki/Key:rcn_ref) 
- `seeAlso[*]`: Liste von URIs, die auf zusätzliche Ressourcen zu diesem Element verweisen.   
- `segregated[boolean]`: Gibt an, ob die Fahrradinfrastruktur physisch von der Fußgängerzone getrennt ist. Modell: [https://wiki.openstreetmap.org/wiki/Key:segregated](https://wiki.openstreetmap.org/wiki/Key:segregated) 
- `source[string]`: Eine Folge von Zeichen, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Es wird empfohlen, den vollständig qualifizierten Domainnamen des Anbieters oder die URL des Quellobjekts zu verwenden   
...
- `type[string]`: NGSI-Entitätstyp. Es muss OSMCycleRoute sein   
...
Hier ist ein Beispiel für eine OSMCycleRoute im JSON-Format als Schlüssel-Wert. Dies ist kompatibel mit NGSI-v2, wenn `options=keyValues` verwendet wird und die Kontextdaten einer einzelnen Entität zurückgibt.  
...
Hier ist ein Beispiel für eine OSMCycleRoute im JSON-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden und die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>Beispiel anzeigen/verstecken</strong></summary>    
...
Hier ist ein Beispiel für eine OSMCycleRoute im JSON-LD-Format als Schlüssel-Wert. Dies ist kompatibel mit NGSI-LD, wenn `options=keyValues` verwendet wird und die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>Beispiel anzeigen/verstecken</strong></summary>    
...
Hier ist ein Beispiel für eine OSMCycleRoute im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden und die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>Beispiel anzeigen/verstecken</strong></summary>    
