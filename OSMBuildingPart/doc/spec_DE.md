...
<!-- /15-License -->  
<!-- 20-Description -->  
Globale Beschreibung: **Ein Teil oder Abschnitt eines Gebäudes aus OpenStreetMap, der mit building:part=* getaggt ist. Wird verwendet, um Gebäude mit unterschiedlichen Höhen, Dachformen oder Materialien in verschiedenen Abschnitten darzustellen und so eine 3D-Gebäudevisualisierung zu ermöglichen (Simple 3D Buildings Standard).**  
Version: 0.0.1  
<!-- /20-Description -->  
<!-- 30-PropertiesList -->  
...
- `alternateName[string]`: Ein alternativer Name für dieses Element  - `areaServed[string]`: Das geografische Gebiet, in dem eine Dienstleistung oder ein angebotener Artikel bereitgestellt wird . Modell: [https://schema.org/Text](https://schema.org/Text)- `buildingColour[string]`: Farbe der Fassade dieses Gebäudeteils, ausgedrückt als CSS-Farbname oder Hex-Code . Modell: [https://wiki.openstreetmap.org/wiki/Key:building:colour](https://wiki.openstreetmap.org/wiki/Key:building:colour)- `buildingHeight[number]`: Gesamthöhe dieses Gebäudeteils in Metern . Modell: [https://wiki.openstreetmap.org/wiki/Key:height](https://wiki.openstreetmap.org/wiki/Key:height)- `buildingLevels[number]`: Anzahl der oberirdischen Stockwerke in diesem Gebäudeteil . Modell: [https://wiki.openstreetmap.org/wiki/Key:building:levels](https://wiki.openstreetmap.org/wiki/Key:building:levels)- `buildingMaterial[string]`: Primäres Fassaden- oder Strukturmaterial dieses Gebäudeteils . Modell: [https://wiki.openstreetmap.org/wiki/Key:building:material](https://wiki.openstreetmap.org/wiki/Key:building:material)- `buildingPartType[string]`: Der Wert des OSM building:part-Tags. Typischerweise 'yes' oder eine spezifische Typbezeichnung für den architektonischen Abschnitt . Modell: [https://wiki.openstreetmap.org/wiki/Key:building:part](https://wiki.openstreetmap.org/wiki/Key:building:part)- `dataProvider[string]`: Eine Zeichenfolge, die den Anbieter der harmonisierten Datenentität identifiziert  - `dateCreated[date-time]`: Zeitstempel der Entitätserstellung. Dieser wird normalerweise von der Speicherplattform zugewiesen  - `dateModified[date-time]`: Zeitstempel der letzten Änderung der Entität. Dieser wird normalerweise von der Speicherplattform zugewiesen  - `description[string]`: Eine Beschreibung dieses Elements  - `id[*]`: Eindeutiger Bezeichner der Entität  - `location[*]`: Geojson-Referenz zum Element. Es kann Point, LineString, Polygon, MultiPoint, MultiLineString oder MultiPolygon sein  - `minHeight[number]`: Höhe über dem Boden, in der dieser Gebäudeteil beginnt, in Metern. Wird für Überhänge und erhöhte Abschnitte verwendet . Modell: [https://wiki.openstreetmap.org/wiki/Key:min_height](https://wiki.openstreetmap.org/wiki/Key:min_height)- `minLevel[number]`: Stockwerk, in dem dieser Gebäudeteil beginnt. Wird für die Innenraumkartierung verwendet . Modell: [https://wiki.openstreetmap.org/wiki/Key:min_level](https://wiki.openstreetmap.org/wiki/Key:min_level)- `name[string]`: Der Name dieses Elements  - `osmId[number]`: Der ursprüngliche OpenStreetMap-Element-Integer-Bezeichner . Modell: [https://wiki.openstreetmap.org/wiki/Elements](https://wiki.openstreetmap.org/wiki/Elements)- `osmLastModified[date-time]`: Zeitstempel der letzten Änderung des OSM-Elements.  - `osmType[string]`: Der Typ des ursprünglichen OpenStreetMap-Elements . Modell: [https://wiki.openstreetmap.org/wiki/Elements](https://wiki.openstreetmap.org/wiki/Elements)- `owner[array]`: Eine Liste, die eine JSON-kodierte Zeichenfolge enthält, die die eindeutigen IDs des/der Eigentümer referenziert  - `refBuilding[uri]`: URI-Referenz zur übergeordneten OSMBuilding-Entität, zu der dieser Teil gehört.  - `roofColour[string]`: Farbe des Daches an diesem Teil, ausgedrückt als CSS-Farbname oder Hex-Code . Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:colour](https://wiki.openstreetmap.org/wiki/Key:roof:colour)- `roofMaterial[string]`: Primäres Material der Dachfläche an diesem Teil . Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:material](https://wiki.openstreetmap.org/wiki/Key:roof:material)- `roofShape[string]`: Form des Daches an diesem Gebäudeteil. Enum:'flat, gabled, hipped, half-hipped, pyramidal, gambrel, mansard, dome, skillion, round, saltbox, onion, cone' . Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:shape](https://wiki.openstreetmap.org/wiki/Key:roof:shape)- `seeAlso[*]`: Liste von URIs, die auf zusätzliche Ressourcen zu diesem Element verweisen.  - `source[string]`: Eine Zeichenfolge, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Es wird empfohlen, den voll qualifizierten Domänennamen des Quellanbieters oder die URL zum Quellobjekt zu verwenden  - `type[string]`: NGSI-Entitätstyp. Es muss OSMBuildingPart sein  <!-- /30-PropertiesList -->  
...
<!-- /50-DataModelHeader -->  
<!-- 60-ModelYaml -->  
<details><summary>**vollständige YAML-Details**</summary>    
```yaml  
OSMBuildingPart:    
...
<!-- 80-Examples -->  
## Beispiel-Payloads    
#### OSMBuildingPart NGSI-v2 Schlüssel-Werte Beispiel    
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-v2, wenn `options=keyValues` verwendet wird, und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary>**Beispiel anzeigen/ausblenden**</summary>    
```json  
...
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden, und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary>**Beispiel anzeigen/ausblenden**</summary>    
```json  
...
#### OSMBuildingPart NGSI-LD Schlüssel-Werte Beispiel    
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-LD-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-LD, wenn `options=keyValues` verwendet wird, und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary>**Beispiel anzeigen/ausblenden**</summary>    
```json  
...
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden, und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary>**Beispiel anzeigen/ausblenden**</summary>    
```json  
