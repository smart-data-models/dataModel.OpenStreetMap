...
<sup><sub>[*] Wenn in einer Eigenschaft kein Typ angegeben ist, kann dies bedeuten, dass sie mehrere Typen oder verschiedene Formate/Muster haben könnte</sub></sup>
...
- `areaServed[string]`: Der geografische Bereich, in dem ein Dienst oder angebotenes Element bereitgestellt wird. Modell: [https://schema.org/Text](https://schema.org/Text)
- `buildingColour[string]`: Farbe der Fassade dieses Gebäudeteils, angegeben als CSS-Farbenname oder Hex-Code. Modell: [https://wiki.openstreetmap.org/wiki/Key:building:colour](https://wiki.openstreetmap.org/wiki/Key:building:colour)
...
- `buildingPartType[string]`: Der OSM building:part-Tag-Wert. Typischerweise 'yes' oder ein spezifisches Typenlabel für den architektonischen Abschnitt. Modell: [https://wiki.openstreetmap.org/wiki/Key:building:part](https://wiki.openstreetmap.org/wiki/Key:building:part)
...
- `minHeight[number]`: Höhe über dem Boden, in der dieser Gebäudeteil beginnt, in Metern. Wird für Überhänge und erhöhte Abschnitte verwendet. Modell: [https://wiki.openstreetmap.org/wiki/Key:min_height](https://wiki.openstreetmap.org/wiki/Key:min_height)
- `minLevel[number]`: Stockwerk, in dem dieser Gebäudeteil beginnt. Wird für Innenraumkartierung verwendet. Modell: [https://wiki.openstreetmap.org/wiki/Key:min_level](https://wiki.openstreetmap.org/wiki/Key:min_level)
...
- `owner[array]`: Eine Liste mit einer JSON-codierten Zeichenfolge, die auf die eindeutigen IDs der Eigentümer verweist
- `refBuilding[uri]`: URI-Referenz auf die übergeordnete OSMBuilding-Entität, zu der dieser Teil gehört
- `roofColour[string]`: Farbe des Daches dieses Teils, angegeben als CSS-Farbenname oder Hex-Code. Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:colour](https://wiki.openstreetmap.org/wiki/Key:roof:colour)
- `roofMaterial[string]`: Primäres Material der Dachfläche dieses Teils. Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:material](https://wiki.openstreetmap.org/wiki/Key:roof:material)
- `roofShape[string]`: Form des Daches dieses Gebäudeteils. Enum: 'flach, Satteldach, Walmdach, Halbwalmdach, Pyramidendach, Krüppelwalmdach, Mansarddach, Kuppel, Pultdach, Runddach, Salzschachtel, Zwiebeldach, Kegel'. Modell: [https://wiki.openstreetmap.org/wiki/Key:roof:shape](https://wiki.openstreetmap.org/wiki/Key:roof:shape)
- `seeAlso[*]`: Liste von URIs, die auf zusätzliche Ressourcen zu diesem Element verweisen
- `source[string]`: Eine Zeichenfolge, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Empfohlen wird die vollqualifizierte Domain des Anbieters oder die URL zum Quellobjekt
- `type[string]`: NGSI-Entitätstyp. Muss OSMBuildingPart sein
<!-- /30-PropertiesList -->
...
Alphabetisch sortiert (zum Anzeigen der Details klicken)
<!-- /50-DataModelHeader -->
<!-- 60-ModelYaml -->
...
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-Format als Schlüssel-Wert-Paar. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und gibt die Kontextdaten einer einzelnen Entität zurück.
...
Hier ist ein Beispiel für ein OSMBuildingPart im JSON-LD-Format als Schlüssel-Wert-Paar. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel ein-/ausblenden</strong></summary>
```json  
