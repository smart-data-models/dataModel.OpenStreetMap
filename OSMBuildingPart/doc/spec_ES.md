...
<!-- /15-License -->  
<!-- 20-Description -->  
Descripción global: **Una parte o sección de un edificio de OpenStreetMap etiquetada con building:part=*. Se utiliza para representar edificios con alturas, formas de tejado o materiales variables en diferentes secciones, lo que permite la visualización de edificios en 3D (estándar Simple 3D Buildings).**  
versión: 0.0.1  
<!-- /20-Description -->  
<!-- 30-PropertiesList -->  
...
- `alternateName[string]`: Un nombre alternativo para este elemento  - `areaServed[string]`: El área geográfica donde se proporciona un servicio o un artículo ofrecido . Modelo: [https://schema.org/Text](https://schema.org/Text)- `buildingColour[string]`: Color de la fachada de esta parte del edificio, expresado como un nombre de color CSS o código hexadecimal . Modelo: [https://wiki.openstreetmap.org/wiki/Key:building:colour](https://wiki.openstreetmap.org/wiki/Key:building:colour)- `buildingHeight[number]`: Altura total de esta parte del edificio en metros . Modelo: [https://wiki.openstreetmap.org/wiki/Key:height](https://wiki.openstreetmap.org/wiki/Key:height)- `buildingLevels[number]`: Número de plantas sobre rasante en esta parte del edificio . Modelo: [https://wiki.openstreetmap.org/wiki/Key:building:levels](https://wiki.openstreetmap.org/wiki/Key:building:levels)- `buildingMaterial[string]`: Material principal de la fachada o estructura de esta parte del edificio . Modelo: [https://wiki.openstreetmap.org/wiki/Key:building:material](https://wiki.openstreetmap.org/wiki/Key:building:material)- `buildingPartType[string]`: El valor de la etiqueta OSM building:part. Típicamente 'yes' o una etiqueta de tipo específica para la sección arquitectónica . Modelo: [https://wiki.openstreetmap.org/wiki/Key:building:part](https://wiki.openstreetmap.org/wiki/Key:building:part)- `dataProvider[string]`: Una secuencia de caracteres que identifica al proveedor de la entidad de datos armonizados  - `dateCreated[date-time]`: Marca de tiempo de creación de la entidad. Normalmente será asignada por la plataforma de almacenamiento  - `dateModified[date-time]`: Marca de tiempo de la última modificación de la entidad. Normalmente será asignada por la plataforma de almacenamiento  - `description[string]`: Una descripción de este elemento  - `id[*]`: Identificador único de la entidad  - `location[*]`: Referencia Geojson al elemento. Puede ser Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `minHeight[number]`: Altura sobre el suelo a la que comienza esta parte del edificio, en metros. Se utiliza para voladizos y secciones elevadas . Modelo: [https://wiki.openstreetmap.org/wiki/Key:min_height](https://wiki.openstreetmap.org/wiki/Key:min_height)- `minLevel[number]`: Nivel de planta en el que comienza esta parte del edificio. Se utiliza para mapeo interior . Modelo: [https://wiki.openstreetmap.org/wiki/Key:min_level](https://wiki.openstreetmap.org/wiki/Key:min_level)- `name[string]`: El nombre de este elemento  - `osmId[number]`: El identificador entero del elemento original de OpenStreetMap . Modelo: [https://wiki.openstreetmap.org/wiki/Elements](https://wiki.openstreetmap.org/wiki/Elements)- `osmLastModified[date-time]`: Marca de tiempo de la última modificación del elemento OSM.  - `osmType[string]`: El tipo del elemento original de OpenStreetMap . Modelo: [https://wiki.openstreetmap.org/wiki/Elements](https://wiki.openstreetmap.org/wiki/Elements)- `owner[array]`: Una lista que contiene una secuencia de caracteres codificada en JSON que hace referencia a los identificadores únicos del(de los) propietario(s)  - `refBuilding[uri]`: Referencia URI a la entidad padre OSMBuilding a la que pertenece esta parte.  - `roofColour[string]`: Color del tejado de esta parte, expresado como un nombre de color CSS o código hexadecimal . Modelo: [https://wiki.openstreetmap.org/wiki/Key:roof:colour](https://wiki.openstreetmap.org/wiki/Key:roof:colour)- `roofMaterial[string]`: Material principal de la superficie del tejado de esta parte . Modelo: [https://wiki.openstreetmap.org/wiki/Key:roof:material](https://wiki.openstreetmap.org/wiki/Key:roof:material)- `roofShape[string]`: Forma del tejado de esta parte del edificio. Enum:'plano, a dos aguas, a cuatro aguas, a media agua, piramidal, gambrel, mansarda, cúpula, a un agua, redondo, saltbox, cebolla, cono' . Modelo: [https://wiki.openstreetmap.org/wiki/Key:roof:shape](https://wiki.openstreetmap.org/wiki/Key:roof:shape)- `seeAlso[*]`: Lista de URIs que apuntan a recursos adicionales sobre este elemento.  - `source[string]`: Una secuencia de caracteres que proporciona la fuente original de los datos de la entidad como una URL. Se recomienda que sea el nombre de dominio completamente cualificado del proveedor de la fuente, o la URL al objeto fuente  - `type[string]`: Tipo de entidad NGSI. Debe ser OSMBuildingPart  <!-- /30-PropertiesList -->  
...
#### Ejemplo de OSMBuildingPart NGSI-v2 key-values    
Aquí hay un ejemplo de un OSMBuildingPart en formato JSON como key-values. Esto es compatible con NGSI-v2 cuando se usa `options=keyValues` y devuelve los datos de contexto de una entidad individual.  
...
Aquí hay un ejemplo de un OSMBuildingPart en formato JSON como normalizado. Esto es compatible con NGSI-v2 cuando no se usan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>mostrar/ocultar ejemplo</strong></summary>    
```json  
...
#### Ejemplo de OSMBuildingPart NGSI-LD key-values    
Aquí hay un ejemplo de un OSMBuildingPart en formato JSON-LD como key-values. Esto es compatible con NGSI-LD cuando se usa `options=keyValues` y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>mostrar/ocultar ejemplo</strong></summary>    
```json  
...
Aquí hay un ejemplo de un OSMBuildingPart en formato JSON-LD como normalizado. Esto es compatible con NGSI-LD cuando no se usan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>mostrar/ocultar ejemplo</strong></summary>    
```json  
