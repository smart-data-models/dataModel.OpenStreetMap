...
**Globale Beschreibung:** **Ein natürliches Merkmal aus OpenStreetMap, das mit natural=* getaggt ist. Beschreibt geografische Merkmale wie Gewässer, Wälder, Gebüsch, Gipfel, Strände usw.**
...
- `areaServed[string]`: Das geografische Gebiet, in dem ein Dienst oder angebotener Artikel bereitgestellt wird. Modell: [https://schema.org/Text](https://schema.org/Text)
...
- `elevation[number]`: Höhe (ele) des Merkmals in Metern über dem Meeresspiegel, typischerweise für Gipfel. Modell: [https://wiki.openstreetmap.org/wiki/Key:ele](https://wiki.openstreetmap.org/wiki/Key:ele)
- `id[*]`: Eindeutige Kennung der Entität
- `leafCycle[string]`: Der Belaubungszyklus von Bäumen oder Wäldern. Modell: [https://wiki.openstreetmap.org/wiki/Key:leaf_cycle](https://wiki.openstreetmap.org/wiki/Key:leaf_cycle)
- `leafType[string]`: Der Belaubungstyp von Bäumen oder Wäldern. Modell: [https://wiki.openstreetmap.org/wiki/Key:leaf_type](https://wiki.openstreetmap.org/wiki/Key:leaf_type)
...
- `naturalType[string]`: Das OSM-Natural-Tag, das das geologische oder physische Merkmal klassifiziert. Enum: 'water, wood, tree, scrub, heath, grassland, peak, ridge, bare_rock, sand, beach, coastline, spring, cave_entrance, wetland...'. Modell: [https://wiki.openstreetmap.org/wiki/Key:natural](https://wiki.openstreetmap.org/wiki/Key:natural)
...
- `type[string]`: NGSI-Entitätstyp. Muss OSMNatural sein
- `water[string]`: Art des Gewässers, wenn natural=water (z. B. See, Stausee, Fluss, Teich). Modell: [https://wiki.openstreetmap.org/wiki/Key:water](https://wiki.openstreetmap.org/wiki/Key:water)
- `wetland[string]`: Art des Feuchtgebiets, wenn natural=wetland (z. B. Sumpf, Moor, Marsch). Modell: [https://wiki.openstreetmap.org/wiki/Key:wetland](https://wiki.openstreetmap.org/wiki/Key:wetland)
<!-- /30-PropertiesList -->
...
## Beispiel-Payloads
#### OSMNatural NGSI-v2 Schlüssel-Wert-Beispiel
Hier ist ein Beispiel für ein OSMNatural im JSON-Format als Schlüssel-Wert-Paar. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und gibt die Kontextdaten einer einzelnen Entität zurück.
...
Hier ist ein Beispiel für ein OSMNatural im JSON-Format als normalisiert. Dies ist kompatibel mit NGSI-v2 ohne Verwendung von Optionen und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel ein-/ausblenden</strong></summary>
```json  
...
#### OSMNatural NGSI-LD Schlüssel-Wert-Beispiel
Hier ist ein Beispiel für ein OSMNatural im JSON-LD-Format als Schlüssel-Wert-Paar. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel ein-/ausblenden</strong></summary>
```json  
...
Hier ist ein Beispiel für ein OSMNatural im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD ohne Verwendung von Optionen und gibt die Kontextdaten einer einzelnen Entität zurück.
<details><summary><strong>Beispiel ein-/ausblenden</strong></summary>
```json  
...
Siehe [FAQ 10](https://smartdatamodels.org/index.php/faqs/) für Informationen zur Behandlung von Maßeinheiten
<!-- /95-Units -->
<!-- 97-LastFooter -->
